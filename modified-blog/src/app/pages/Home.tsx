...
const projects = [
  ...
  },
  {
    ...
    desc: '심해와 고래 테마의 개인 개발 블로그. React + Framer Motion 기반 몰입형 UX.',
...
function SectionHead({ num, title, italic }: { num: string; title: string; italic: string }) {
  return (
    <div className="mb-14">
      ...
      <h2
        className="font-bold leading-none tracking-tight text-[#c4ddf0]"
        style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(34px, 5vw, 58px)', letterSpacing: '-0.03em' }}
      >
...
export function Home() {
  ...
  const [whaleQuote, setWhaleQuote] = useState('');
  const [showQuote, setShowQuote] = useState(false);
  const quotes = ['오늘도 깊이 다이브 중... 🫧', '버그를 삼켜버리겠어! 🐋', 'git push origin ocean 🌊', '코드의 바다에서 헤엄치는 중~'];
  const qIdxRef = useRef(0);

  const handleWhaleClick = () => {
    setWhaleQuote(quotes[qIdxRef.current % quotes.length]);
    qIdxRef.current++;
    setShowQuote(true);
    setTimeout(() => setShowQuote(false), 2400);
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen overflow-x-hidden" style={{ background: DEPTH.surface }}>
        <SwimmingWhales />
        ...
        <section
          ...
          style={{ background: `linear-gradient(160deg, #071828 0%, #03080e 55%)` }}
        >
          {/* grid bg */}
          <div
            ...
            style={{
              backgroundImage:
                'linear-gradient(rgba(196,221,240,.022) 1px,transparent 1px),linear-gradient(90deg,rgba(196,221,240,.022) 1px,transparent 1px)',
          ...
          <div
            className="absolute inset-0 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse 75% 85% at 50% 50%,transparent 25%,#03080e 100%)' }}
          />

          {/* whale — right side */}
          <div className="absolute right-[6%] top-0 bottom-0 flex items-center pointer-events-none z-10">
            <div
            ...
            >
              ...
              <motion.div
                ...
                style={{ backgroundColor: 'rgba(189,221,252,0.92)', color: '#1a3a52', fontFamily: "'Space Mono', monospace" }}
                initial={false}
                animate={{ opacity: showQuote ? 1 : 0, y: showQuote ? -14 : 0, scale: showQuote ? 1 : 0.85 }}
                transition={{ type: 'spring', stiffness: 300, damping: 22 }}
              >
                {whaleQuote}
                <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 rotate-45" style={{ backgroundColor: 'rgba(189,221,252,0.92)' }} />
              </motion.div>
            </div>
          </div>
          ...
          >
            <motion.p
              className="text-[9px] tracking-[0.42em] mb-4"
              style={{ color: 'rgba(196,221,240,0.32)' }}
            ...
            <motion.h1
              className="font-extrabold leading-[.9] tracking-tight text-[#c4ddf0] mb-5"
              style={{
            ...
            >
              개발자<br />
              <em className="not-italic" style={{ color: '#5ba3cc' }}>이름</em>
            </motion.h1>
            ...
            >
              {[
                ...
                <a
                  ...
                  className="flex items-center gap-2 text-[9px] tracking-[0.22em] border px-4 py-2.5 transition-all duration-300 uppercase"
                  style={{
                    color: 'rgba(196,221,240,0.35)',
                    borderColor: 'rgba(196,221,240,0.12)',
                  }}
                  onMouseEnter={e => {
                    (e.currentTarget as HTMLElement).style.color = '#c4ddf0';
                    (e.currentTarget as HTMLElement).style.borderColor = 'rgba(196,221,240,0.38)';
                    (e.currentTarget as HTMLElement).style.background = 'rgba(196,221,240,0.04)';
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLElement).style.color = 'rgba(196,221,240,0.35)';
                    (e.currentTarget as HTMLElement).style.borderColor = 'rgba(196,221,240,0.12)';
          ...
          >
            <span className="text-[8px] tracking-[0.4em]" style={{ color: 'rgba(196,221,240,0.18)' }}>scroll</span>
            <motion.div
              className="w-px h-11"
              style={{ background: 'linear-gradient(transparent, rgba(196,221,240,0.25))' }}
        ...
        {/* ══ CAREER ══ */}
        <section style={{ background: `linear-gradient(180deg, ${DEPTH.surface} 0%, ${DEPTH.shallow} 100%)` }}>
          <div className="max-w-[940px] mx-auto px-11 py-20">
                ...
                <Reveal key={i} delay={i * 0.1} x={-16}>
                  <div
                    ...
                    style={{
                      ...
                      borderTop: '1px solid rgba(196,221,240,0.06)',
                    }}
                  >
                    <p className="text-[9px] leading-loose tracking-[0.12em]" style={{ color: 'rgba(196,221,240,0.22)', paddingTop: 3 }}>
                      {item.date}
                    </p>
                    <div>
                      <p className="text-[19px] font-bold text-[#c4ddf0] mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>{item.name}</p>
                      <p className="text-[9px] tracking-[0.2em] mb-2.5" style={{ color: '#5ba3cc' }}>{item.role}</p>
                      <p className="text-[11px] leading-loose" style={{ color: 'rgba(196,221,240,0.36)' }}>{item.desc}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
        ...
        {/* ══ PROJECTS ══ */}
        <section style={{ background: `linear-gradient(180deg, ${DEPTH.shallow} 0%, ${DEPTH.mid} 100%)` }}>
          <div className="max-w-[940px] mx-auto px-11 py-20">
            ...
            <div
              ...
              style={{
                ...
                gap: '1px',
                background: 'rgba(196,221,240,0.06)',
              }}
            >
              {projects.map((p, i) => (
                <motion.div
                  ...
                  style={{ background: DEPTH.shallow, padding: '32px 28px' }}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, margin: '-40px' }}
                  transition={{ delay: i * 0.1, duration: 0.65, ease: [0.16, 1, 0.3, 1] }}
                  whileHover={{ background: '#070f1a' }}
                >
                  ...
                  <motion.p
                    className="text-[19px] font-bold text-[#c4ddf0] mb-3 leading-snug"
        ...
        {/* ══ BLOG ══ */}
        <section style={{ background: `linear-gradient(180deg, ${DEPTH.mid} 0%, ${DEPTH.deep} 100%)` }}>
          <div className="max-w-[940px] mx-auto px-11 py-20">
                ...
                <Reveal key={i} delay={i * 0.08}>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <motion.p
                        className="text-[15px] font-bold text-[#c4ddf0] mb-1 leading-snug"
                        style={{ fontFamily: "'Syne', sans-serif" }}
                        whileHover={{ color: '#5ba3cc' }}
